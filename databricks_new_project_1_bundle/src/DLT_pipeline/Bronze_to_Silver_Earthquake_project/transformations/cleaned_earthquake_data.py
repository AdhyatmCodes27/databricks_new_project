...
volume_path = f'/Volumes/{catalog_name}/bronze/earthquake_data_1'
...
def earthquake_data():
    df = spark.readStream.format('cloudfiles').option('cloudfiles.format', 'json').load(volume_path).withColumn("_loaded_timestamp", current_timestamp())

    df = df.withColumn("parsed_data", from_json(col("features"), schema))

    df = df.select(explode(col("parsed_data")).alias("features"), "_loaded_timestamp")

    df = df.select("features.properties.*", "features.id", col("features.geometry.coordinates")[0].alias("longitude"), col("features.geometry.coordinates")[1].alias("latitude"), col("features.geometry.coordinates")[2].alias("depth"), "_loaded_timestamp")
